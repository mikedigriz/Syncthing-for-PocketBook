...
- **Статус при работе**: отправляет запрос через unix-сокет `/tmp/syncthing.sock`, а не TCP. Скрипт сидит в сетевой песочнице и не видит ни `127.0.0.1`, ни сеть, только файлы и сокеты.
- **Логирование**: в `syncthing.log` записываются только строки с `ERR`, иначе файл лога раздуется до бесконечности на встроенном хранилище.
...
> [!IMPORTANT]\
> Все файлы скриптов (`.app`, `.sh`) ДОЛЖНЫ использовать **LF** (Line Feed, `\n`), а не CRLF (Carriage Return + Line Feed, `\r\n`).

На Windows по умолчанию Git конвертирует LF в CRLF и обратно, но скрипты на PocketBook требуют строго LF. Если использовать CRLF, скрипты не будут работать.
...
Git автоматически преобразует окончания в соответствии с [.gitattributes](.gitattributes), но лучше убедиться в настройках редактора.
