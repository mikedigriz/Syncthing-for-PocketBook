...
> [!WARNING]\
> `syncthing_pro.app` при запуске ставит блокировку, а снимает её только `syncthing_kill.app`. Поэтому останавливайте им. Если остановить иначе (например выключить Wi-Fi), блокировка останется, и следующий тап по `syncthing_pro.app` молча ничего не сделает, пока не перезагрузите устройство. Подробнее в разделе [«Блокировка»](SCRIPTS.md#блокировка).
