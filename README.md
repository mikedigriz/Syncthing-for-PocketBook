...
Папка `system` с файлом `view.json` скрытая. Единого способа показать скрытые файлы для всех ОС нет, но в каждой это одно действие:
...
Дальше всё [как и на других устройствах с Syncthing](https://docs.syncthing.net/intro/getting-started.html):
...
> [!WARNING]\
> `syncthing_pro.app` при запуске ставит блокировку (`mkdir /tmp/syncthing.lock`), чтобы не плодить второй процесс, а снимает её `syncthing_kill.app` строкой `rmdir /tmp/syncthing.lock`. Поэтому останавливайте только им. Если остановить иначе (например просто выключить Wi-Fi), блокировка останется, и следующий тап по `syncthing_pro.app` молча ничего не сделает, пока не удалите папку вручную или не перезагрузите устройство. Подробнее в разделе [«Блокировка»](SCRIPTS.md#блокировка).

> [!WARNING]\
> После этой правки веб-панель по адресу `http://your-ip-address:8384` перестанет открываться. Поэтому все папки настройте заранее. Захотите вернуть панель, просто верните обратно адрес `0.0.0.0:8384`.
